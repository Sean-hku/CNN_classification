...
class _BaseWrapper(object):
    ...
    def forward(self, image):
        ...
        self.probs = F.softmax(self.logits, dim=1)#socre--->softmax
        # print(self.probs)
    ...
    def backward(self, ids):
        ...
        self.logits.backward(gradient=one_hot, retain_graph=True)#只反传类别信息 hook已将grad取得，retain会累计梯度
...
class GradCAM(_BaseWrapper):
    ...
    def __init__(self, model, candidate_layers=None):
        ...
        for name, module in self.model.named_modules():
            if self.candidate_layers is None or name in self.candidate_layers:
                self.handlers.append(module.register_forward_hook(forward_hook(name)))   #hook获取一些中间结果，如某层的输出或者梯度
    ...
    def forward(self, image):
        ...
        return super(GradCAM, self).forward(image)# 执行父类的方法
